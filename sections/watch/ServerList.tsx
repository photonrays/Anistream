            ...
            {serverList?.sub && serverList.sub.length > 0 && <div className='grid grid-cols-[90px_1fr]'>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    {serverList.sub.map((s, index) => (
                        <button onClick={() => handleChangeServer(s, "sub")} className={`${server?.category === 'sub' && server.serverName === s.serverName ? "bg-primary" : "bg-card"} py-2 px-5 rounded-md text-sm`} key={index}>
                            {s.serverName}
                        </button>))}
                </div >
            </div>}
            {serverList?.dub && serverList.dub.length > 0 && <div className='grid grid-cols-[90px_1fr]'>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    {serverList.dub.map((s, index) => (
                        <button onClick={() => handleChangeServer(s, "dub")} className={`${server?.category === 'dub' && server?.serverName === s.serverName ? "bg-primary" : "bg-card"} py-2 px-5 rounded-md text-sm`} key={index}>
                            {s.serverName}
                        </button>))}
                </div >
            </div>}
            {serverList?.raw && serverList.raw.length > 0 && <div className='grid grid-cols-[90px_1fr]'>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    {serverList.raw.map((s, index) => (
                        <button onClick={() => handleChangeServer(s, "raw")} className={`${server?.category === 'raw' && server.serverName === s.serverName ? "bg-primary" : "bg-card"} py-2 px-5 rounded-md text-sm`} key={index}>
                            {s.serverName}
                        </button>))}
                </div >
            </div>}
